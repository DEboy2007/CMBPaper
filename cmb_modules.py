...
def make_CMB_T_map(N, pix_size, ell, DlTT):
    ...
    onesvec = np.ones(N)
    # create an array of size N between -0.5 and +0.5
    ...
    # fill in the Cls until the max of the ClTT vector
...
def Plot_CMB_Map(Map_to_Plot, c_min, c_max, X_width, Y_width, title):
    from mpl_toolkits.axes_grid1 import make_axes_locatable
    plt.title(title)
    print("map mean:", np.mean(Map_to_Plot), "map rms:", np.std(Map_to_Plot))
    plt.gcf().set_size_inches(10, 10)
    im = plt.imshow(Map_to_Plot, interpolation='bilinear',
                    origin='lower', cmap=cm.RdBu_r)
...
def SZ_source_component(N, pix_size, Number_of_SZ_Clusters, Mean_Amplitude_of_SZ_Clusters, SZ_beta, SZ_Theta_core, do_plots):
    ...
    while (i < Number_of_SZ_Clusters):
        pix_x = np.int(N*np.random.rand())
        pix_y = np.int(N*np.random.rand())
        pix_amplitude = np.random.exponential(
...
def make_noise_map(N, pix_size, white_noise_level, atmospheric_noise_level, one_over_f_noise_level):
    ...
    if (atmospheric_noise_level != 0):
        ...
        R = np.sqrt(X**2. + Y**2.) * pix_size / \
            60.  # angles relative to 1 degrees
        ...
        atmospheric_noise = np.fft.ifft2(
            atmospheric_noise * np.fft.fftshift(mag_k**(5/3.))) * atmospheric_noise_level/pix_size
...
#    print FMap
 ...
 #   print PSMap
    ...
    while (i < N_bins):
        ...
        inds_in_bin = ((ell2d >= (i * delta_ell)) *
                       (ell2d < ((i+1) * delta_ell))).nonzero()
